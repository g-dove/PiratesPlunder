...
- **LootService.lua** (`PP.Loot`) — `Post`, `Cancel`, `Award`, `SubmitResponse`, `PostAll`. Raid leader runs an idle-clear timer (`_ScheduleIdleClear` / `_CancelIdleClear`): 60 s after the queue empties, broadcasts `LOOT_CLEAR` so all clients wipe stuck response frames. `Award` carries `rosterVersion` and a minimal `session` shell on the `LOOT_AWARD` payload so receivers update score, advance their version, and install the session record if missing — a version gap triggers `RequestSessionSync` to converge on the leader.
...
- **Sync.lua** — all inter-client communication via AceComm-3.0 over the `"PPLNDR"` prefix. All messages send through `SendAddonMessage` directly; there is no `BroadcastCritical` / ACK-echo machinery. Roster and loot broadcasts (`ROSTER_UPDATE`, `GROUP_SCORE`, `LOOT_POST`, `LOOT_AWARD`, `LOOT_CANCEL`, plus `SESSION_CREATE` / `CLOSE` / `DELETE`) carry `(guildKey, activeSessionID, activeSessionVersion)`; receivers run `PP:_adoptSessionContext` first to converge on the leader's session even if they missed `SESSION_CREATE`. `LOOT_POST` and `LOOT_AWARD` additionally carry a minimal `session = { name, startTime, leader, guildKey }` shell so receivers can install a session record via `PP:_ensureSessionRecord` before showing the response popup or recording an award. Version-gap detection on `LOOT_AWARD` (`rosterVersion`) schedules `RequestSessionSync`; equal versions imply equal state since every roster mutation broadcasts the full table. Duplicate-delivery guards: `_completedLootKeys` for awarded/cancelled/cleared loot keys; the salt prefix on every payload defeats AceComm first-byte dedupe. **Two sync paths:** (1) `RequestSync` / `SYNC_FULL` (multi-guild dump, manual escape hatch behind the Settings "Request Sync" button); (2) `RequestSessionSync` / `SESSION_SYNC_REQUEST` / `SESSION_SYNC_REPLY` (leader-only, single-guild reply with roster + active session + live `pendingLoot` + `_completedLootKeys`, broadcast-only with a 5 s cooldown that coalesces concurrent requesters into one reply). Internal callers (`_ScheduleJoinSync`, `CompletePendingSessionEnd`, `OnGuildRosterUpdate`, etc.) use the session-sync path. `SendFullSync(guildKey, target?)` accepts an optional whisper target that bypasses the 10 s broadcast cooldown. Whispered `SYNC_FULL` is only trusted to introduce new guild keys within 15 s of our own `RequestSync` from a current group member. Snapshots are exchanged inline on `SESSION_CLOSE` and on demand via `SNAPSHOT_REQUEST` / `SNAPSHOT_REPLY` (BULK); replies are accepted only from current group members in response to a recent local request. Most loot/session/sync messages send at `NORMAL`; `LOOT_STATE_QUERY`, `RAID_SETTINGS`, the `SNAPSHOT_*` pair, and `SESSION_SYNC_REPLY` send at `BULK`. Post-join handshake retries `RequestSessionSync` up to three times (3 s / 9 s / 15 s) and stops once an active session is adopted. `LOOT_CLEAR` is accepted only from the current raid leader; cleared keys go into `_completedLootKeys` so a delayed `LOOT_POST` cannot re-pop a wiped item. Session-broadcast gates: `BroadcastSessionCreate` / `Close` require `IsRaidLeader()`; `BroadcastSessionDelete` requires `CanModify()` (officer rank) since deletes can run outside a raid.
...
Message types include `ACK`, `GROUP_SCORE`, `LOOT_CLEAR`, `SNAPSHOT_REQUEST` / `SNAPSHOT_REPLY`, and `SESSION_SYNC_REQUEST` / `SESSION_SYNC_REPLY` in addition to the original set. Most loot/session/sync messages send at `NORMAL` AceComm priority; `LOOT_STATE_QUERY`, `RAID_SETTINGS`, the `SNAPSHOT_*` pair, and `SESSION_SYNC_REPLY` send at `BULK`. `PP._ppUsers` tracks which group members have the addon loaded (populated on any incoming message, cleared on group leave). `PP._completedLootKeys` tracks loot keys that have been awarded/cancelled/cleared so late `LOOT_POST` messages cannot re-pop them.
